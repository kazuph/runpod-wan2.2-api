...
from comfy_extras import nodes_wan, nodes_flux, nodes_model_sampling

# Initialize nodes for WAN2.2 workflow
DiffusionModelLoader = nodes_flux.NODE_CLASS_MAPPINGS["DiffusionModelLoader"]()
Wan2VAELoader = nodes_wan.NODE_CLASS_MAPPINGS["Wan2VAELoader"]()
...
ModelSamplingFlux = nodes_model_sampling.NODE_CLASS_MAPPINGS["ModelSamplingFlux"]()

# Load models
with torch.inference_mode():
    # Load WAN2.2 models
    unet = DiffusionModelLoader.load_diffusion_model("wan2.2_ti2v_5B_fp16.safetensors", "fp16")[0]
    vae = Wan2VAELoader.load_vae("wan2.2_vae.safetensors")[0]
    clip = DualCLIPLoader.load_clip("umt5_xxl_fp8_e4m3fn_scaled.safetensors", "flux", local_files_only=False)[0]
...
def generate(input):
    ...
    try:
        ...
        # Model sampling
        model = ModelSamplingFlux.patch(unet, 1.1, height // 8, width // 8)[0]
        
        if is_t2i:
            print(f"Generating T2I (text-to-image) with prompt: {positive_prompt}")
            print(f"Resolution: {width}x{height}, Seed: {seed}")
            
            # T2I workflow - create empty latent
            latent = EmptyLatentImage.empty_latent_image(width, height, 1)[0]
            
        else:
            print(f"Generating I2I (image-to-image) with prompt: {positive_prompt}")
            print(f"Resolution: {width}x{height}, Seed: {seed}")
            
            # I2I workflow - encode input image
            input_image_path = get_input_image_path(input_image_param)
            input_image = LoadImage.load_image(input_image_path)[0]
            latent = VAEEncode.encode(vae, input_image)[0]
        
        # Encode prompts
        conditioning = CLIPTextEncode.encode(clip, positive_prompt)[0]
        conditioning = FluxGuidance.append(conditioning, 4.0)[0]
        ...
        # Run sampling
        samples = SamplerCustomAdvanced.sample(noise, guider, sampler_name, sigmas, latent)[0]
